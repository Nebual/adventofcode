...
""".strip()
    ...
    def printout(self, spots):
        ...
        for i, spot in enumerate([
            ...
            s[1][i+1] = spots[spot] if spot in spots and spots[spot] else '.'
        s[2][3] = spots['room_a1'] if 'room_a1' in spots and spots['room_a1'] else '.'
        s[3][3] = spots['room_a2'] if 'room_a2' in spots and spots['room_a2'] else '.'
        s[2][5] = spots['room_b1'] if 'room_b1' in spots and spots['room_b1'] else '.'
        s[3][5] = spots['room_b2'] if 'room_b2' in spots and spots['room_b2'] else '.'
        s[2][7] = spots['room_c1'] if 'room_c1' in spots and spots['room_c1'] else '.'
        s[3][7] = spots['room_c2'] if 'room_c2' in spots and spots['room_c2'] else '.'
        s[2][9] = spots['room_d1'] if 'room_d1' in spots and spots['room_d1'] else '.'
        s[3][9] = spots['room_d2'] if 'room_d2' in spots and spots['room_d2'] else '.'
    ...
    def isDone(self, spots):
        for room in Layout.ROOMS:
            if not spots[room] or spots[room] != room[5]:
                return False
        return True
    def getPart1(self):
    ...
    def runSim(self, spots, energy = 0):
        ...
        for spot, bug in spots.items():
            if not bug: continue
            correctRoom2 = f"room_{bug}2"
            if spot[0:6] == 'room_' + bug: # in right room
                if spot[-1] == '2':
                    # del spots[spot]
                    continue # in correct room bottom
                if spots[correctRoom2] == bug:
                    # del spots[spot]
            ...
            for destSpot, destBug in spots.items():
                if destBug:
                    continue # full
                if spot[0:4] == "hall" and destSpot[0:4] == "hall":
                    continue # can't move in halls twice
                if destSpot[0:4] == "room":
                    if destSpot[5] != bug:
                        continue # wrong room
                    if destSpot[6] == "1" and spots[correctRoom2] != bug:
                        continue # don't move into spot 1 if spot 2 is empty/wrong
                # fork and move
